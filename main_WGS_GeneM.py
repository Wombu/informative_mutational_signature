...
from src.Dataset import FeatureDataset, Dataset
...
path_data = f"data/WGS_GeneM"
data_name = "dataset_WGS_GeneM"
label_name = "label_WGS_GeneM"

data = util.import_data_pd(filename=f"{path_data}/{data_name}.csv")
df_label = util.import_data_pd(filename=f"{path_data}/{label_name}.csv")

feature_names = list(data.index)
...
for i in range(1, 11, 1):
    ann = torch.load(f"{path_data}/ann{i}.pt")  # import individual model

    validation_loader = torch.load(f"data/WGS_MS+Bins/val_loader{i}.pth") # import validation dataset

    indizes = validation_loader.batch_sampler.sampler.indices
    _, label = validation_loader.dataset[indizes]
    label = list(label.numpy())
    dataset = Dataset(X=data.iloc[:, indizes], Y=df_label.iloc[:, indizes], one_hot=True)

    predictions = []  # predictions
    labels = []  # true label
    for d, data_tmp in enumerate(dataset):
        X, Y = data_tmp
        composite = EpsilonPlus()  # uses epsilon rule for all fully connected layers
        with Gradient(model=ann, composite=composite, attr_output=util.one_hot_max) as attributor:
            out, relevance = attributor(input=X)
            out = out[0]
...
for pred_label in relevances.keys():
    ...
    for i in range(shape[0]):
        ...
        importance_limit = relevances_pred[pred_label][i] * quantitative_lrp_threshold  # threshold for the individual prediction
